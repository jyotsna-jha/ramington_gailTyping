    ...
    if (timeLeft > 0) {
      ...
      for (let i = 0; i < inputValue.length; i++) {
        ...
        if (char in charactersMapping) {
          ...
          newInput.endsWith(characterMapping["fe"])
        ) {
          // Append the mapping of the current character to newInput
          newInput += characterMapping[char] || char;
        } else if (
          ...
          newInput = newInput.slice(0, -characterMapping["f"].length);
          newInput += characterMapping["fe"];
        } 
       /*  else if (
          (char === "[" || char === "A" || char === "k") &&
          (newInput.endsWith(characterMapping["f[k"]) ||
            newInput.endsWith(characterMapping["f[A"]))
        ) {
          // Handle '[' or 'A' or 'k' after 'f[k' or 'f[A'
          newInput += characterMapping[char] || char;
        } */
         else if (
          (char === "[" || char === "A" || char === "k") &&
